...
class LibroController extends Controller
{
    ...
    {
        $model = $this->findModel($id);
        // preguntamos si existe la imagen en 
        // la carpeta uploads
        ...
        if ($model->load($this->request->post())) {
            ...
            if ($model->validate()) {
                if ($model->archivo) {
                    ...
                    $ruteArchivo = 'uploads/' . time() . '_' . $model->archivo->baseName . '.' . $model->archivo->extension;
                    // guardando archivo en carpeta uploads
                    if ($model->archivo->saveAs($ruteArchivo)) {
                        $model->image = $ruteArchivo;
                    }
                }
            }
            if ($model->save(false)) {
